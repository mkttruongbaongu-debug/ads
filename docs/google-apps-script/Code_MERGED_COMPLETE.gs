 ...
 * 
 * CÁCH DEPLOY:
 * 1. Tạo Google Sheet mới hoặc mở sheet hiện tại
...
function saveTaiKhoan(data) {
    try {
        ...
        if (existingRowIndex > 0) {
            ...
            logAction('saveTaiKhoan', fbUserId, '', 1, 'updated', 'TAI_KHOAN updated');
        } else {
            sheet.appendRow(rowData);
            logAction('saveTaiKhoan', fbUserId, '', 1, 'created', 'TAI_KHOAN created');
        }
